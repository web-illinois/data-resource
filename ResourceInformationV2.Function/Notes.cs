...
public class Notes {
    ...
    private readonly NoteGetter _noteGetter;
    private readonly ApiHelper _apiHelper;
    private readonly LogHelper _logHelper;
    private readonly NoteSetter _noteSetter;

    public Notes(ILogger<Notes> logger, NoteGetter noteGetter, ApiHelper apiHelper, LogHelper logHelper, NoteSetter noteSetter) {
        _logger = logger;
        _noteGetter = noteGetter;
        _apiHelper = apiHelper;
        _logHelper = logHelper;
        _noteSetter = noteSetter;
    }

    [Function("NoteLoad")]
    [OpenApiOperation(operationId: "NoteLoad", tags: "Notes", Description = "Load a note by API. This will be put in draft mode unless instructed by the Administration application.")]
    [OpenApiParameter(name: "ilw-key", In = ParameterLocation.Header, Required = true, Type = typeof(string), Description = "The API Key.")]
    [OpenApiRequestBody(contentType: "application/json", bodyType: typeof(NoteItem), Required = true, Description = "A json implementation of a note. An ID will be generated automatically if it isn't created, and it will error out if the ID doesn't start with the source plus a '-' value.")]
    [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "text/plain", bodyType: typeof(string), Description = "The ID of the note that was loaded.")]
    public async Task<HttpResponseData> Load([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req) {
        try {
            _logger.LogInformation("Called NoteLoad.");
            var requestHelper = RequestHelperFactory.Create();
            requestHelper.Initialize(req);
            var key = requestHelper.GetCodeFromHeader(req);
            var item = await req.ReadFromJsonAsync<NoteItem>() ?? new NoteItem();
            ...
            await response.WriteAsJsonAsync(results.forceDraft
                ? await _noteSetter.SetItemWithDraft(item)
                : await _noteSetter.SetItem(item));
            await _logHelper.Log(CategoryType.Note, FieldType.None, "API", item.Source, item, "API Load", EmailType.OnSubmission);
            return response;
        } catch (Exception ex) {
    ...
    [OpenApiOperation(operationId: "NoteFragment", tags: "Notes", Description = "Note by a fragment.")]
    ...
    [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "text/plain", bodyType: typeof(NoteItem), Description = "The note. If the note is not found, it will be blank.")]
    public async Task<HttpResponseData> GetByFragment([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req) {
        _logger.LogInformation("Called NotesFragment.");
        var requestHelper = RequestHelperFactory.Create();
        requestHelper.Initialize(req);
    ...
    [Function("Note")]
    [OpenApiOperation(operationId: "Note", tags: "Notes", Description = "Get a specific note.")]
    ...
    public async Task<HttpResponseData> GetById([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req) {
        _logger.LogInformation("Called Note.");
        var requestHelper = RequestHelperFactory.Create();
        requestHelper.Initialize(req);
    ...
    [Function("NoteSearch")]
    [OpenApiOperation(operationId: "NoteSearch", tags: "Notes", Description = "Search note by a specific source. The search can include both a free-query text search and filter list.")]
    ...
    [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "text/plain", bodyType: typeof(SearchObject<NoteItem>), Description = "The list of notes")]
    public async Task<HttpResponseData> Search([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req) {
        _logger.LogInformation("Called NoteSearch.");
        var requestHelper = RequestHelperFactory.Create();
        requestHelper.Initialize(req);
        var source = requestHelper.GetRequest(req, "source");
        ...
        requestHelper.Validate();
        var response = req.CreateResponse(HttpStatusCode.OK);
        await response.WriteAsJsonAsync(await _noteGetter.Search(source, query, tags, tags2, tags3, tags4, topics, audience, department, take, skip, sort));
        return response;
    }
}
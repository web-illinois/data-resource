...
namespace ResourceInformationV2.Search.Setters
{
    ...
    public abstract class BaseSetter<T>(OpenSearchClient? openSearchClient) where T : BaseObject, new()
    {
        ...
        {
            DeleteResponse response = await _openSearchClient.DeleteAsync<T>(id, i => i.Index(IndexName));
        ...
        public async Task<string> PublishDraftItem(T item)
        {
            ...
            item.Prepare();
            IndexResponse response = await _openSearchClient.IndexAsync(item, i => i.Index(IndexName));
            if (response.IsValid)
            {
                DeleteResponse responseDelete = await _openSearchClient.DeleteAsync<T>(oldId, i => i.Index(IndexName));
        ...
        public async Task<string> SetItemWithDraft(T item)
        {
            ...
            {
                GetResponse<T> responseOriginalItem = await _openSearchClient.GetAsync<T>(item.Id);
                ...
                {
                    T originalItem = responseOriginalItem.Source;
                    originalItem.IsDraftAvailable = true;
                    item.Id = item.Id.Replace(item.Source + "-", item.Source + "!-");
                    IndexResponse responseOriginalSave = await _openSearchClient.IndexAsync(originalItem, i => i.Index(IndexName));
                }
            }
            item.IsActive = false;
            item.CreatedOn = DateTime.Now;
        ...
        public async Task<string> UploadFile(string source, string file, bool validateTags, IEnumerable<string> tag1, IEnumerable<string> tag2, IEnumerable<string> tag3, IEnumerable<string> tag4, IEnumerable<string> audience, IEnumerable<string> topic, IEnumerable<string> department)
        {
            ...
            foreach (var line in lines)
            {
                lineNumber++;
                var item = new T();
                (bool successful, bool headerIssue, string? message) = item.LoadFromString(source, line);
                if (successful)
                {
                    if (validateTags)
                    {

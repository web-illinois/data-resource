...
namespace ResourceInformationV2.Search.Helpers {
    ...
    public class BulkEditor(OpenSearchLowLevelClient? openSearchLowLevelClient) {
        ...
        public async Task<string> DeleteAllItems(string source) {
            ...
            var responseFaqs = await _openSearchLowLevelClient.DeleteByQueryAsync<StringResponse>(UrlTypes.Faqs.ConvertToUrlString(), GenerateDeleteJson(source));
            var responseEvents = await _openSearchLowLevelClient.DeleteByQueryAsync<StringResponse>(UrlTypes.Events.ConvertToUrlString(), GenerateDeleteJson(source));
        ...
        public async Task<string> UpdateTags(string source, string listName, string oldTag, string newTag) {
            ...
            var responseFaqs = await _openSearchLowLevelClient.UpdateByQueryAsync<StringResponse>(UrlTypes.Faqs.ConvertToUrlString(), GenerateUpdateJson(source, listName, oldTag, newTag));
            var responseEvents = await _openSearchLowLevelClient.UpdateByQueryAsync<StringResponse>(UrlTypes.Events.ConvertToUrlString(), GenerateUpdateJson(source, listName, oldTag, newTag));
            return responseResources.Success && responsePublications.Success && responsePeople.Success && responseNotes.Success && responseFaqs.Success && responseEvents.Success ? $"{source} updated: tag {oldTag} to {newTag}" : source + " not updated";
        }
